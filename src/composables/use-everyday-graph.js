...
export default function(store, graphElementId, props) {
    ...
    const splitData = rawData => {
        ...
        let allData = indicators.EVERYDAY.calculate(dailyData);

        let sqzState = allData[31];
        let mtmState = allData[32];
        let mtmVal = allData[33];
        let longCond = allData[0];
        let shortCond = allData[1];

        let trends = indicators.TTMTrend.calculate(dailyData, {
            type: "HA"
        });
        let ema50 = indicators.MA.calculate(dailyData, {
            n: 50,
            type: "ema",
            source: "close",
            digits: 3
        });
        let ema8 = indicators.MA.calculate(dailyData, {
            n: 8,
            type: "ema",
            source: "close",
            digits: 3
        });
        let ema21 = indicators.MA.calculate(dailyData, {
            n: 21,
            type: "ema",
            source: "close",
            digits: 3
